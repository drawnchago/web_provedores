...
import { PurchaseOrderDetail } from 'src/app/shared/Interfaces/PurchaseOrderDetail';
import { PurchaseOrder } from 'src/app/shared/Interfaces/Purchase_Order';
import { User } from 'src/app/shared/Interfaces/user';
import { AuthService } from 'src/app/shared/services/auth.service';
import { PurchaseRequisitionsDialogComponent } from '../purchase-requisitions/purchase-requisitions-dialog/purchase-requisitions-dialog.component';
import { Conversation, Levels, PurchaseRequsition, PurchaseRequsitions, RequistionDetails } from '../purchase-requisitions/purchase-requisitions.model';
...
export class PurchaseOrderComponent implements OnInit {
  ...
  //ARRAY
  public purchase_orders: PurchaseOrder[];
  ...
  messageDiaglog(element, type) {
    if (type == 1) {
      ...
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        data: {
          message: '¿Estas seguro que deseas aprobar la orden de compra: ' + element.id + '?',
          name: element.name
        }
      }).afterClosed()
        .subscribe((confirm: Boolean) => {
          if (confirm) {
            this.authorizeOrDeny(element, 1);
          } else {
            this.toastr.warning('Cancelado');
          }
        });
      ...
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        data: {
          message: '¿Estas seguro que deseas rechazar la orden de compra: ' + element.id + '?',
          name: element.name
        }
      }).afterClosed()
        .subscribe((confirm: Boolean) => {
          if (confirm) {
            this.authorizeOrDeny(element, 2);
          } else {
            this.toastr.warning('Cancelado');
          }
        });
  ...
  authorizeOrDeny(element, type) {
    const data = {
      id: element.id,
      type: type,
      user_id: this.user.id
    }
    this.services.authorizeOrDeny(data).subscribe(response => {
  ...
  getPurchaseOrderDetails(obj: PurchaseOrderDetail, to_print) {
    this.id = obj.id;
    this.created_by = obj.created_by;
    this.created_at = obj.created_at;
    this.status = obj.status;
    this.services.getPurchaseOrdersDetails(obj.id).subscribe(response => {

      if (!response['success']) {
        this.toastr.error(response['message']);
      }

      this.requisition_details = response['purchase_order_details'];
  ...
  load() {
    ...
    this.services.getPurchaseOrders().subscribe(response => {
      ...
      this.dataSource = new MatTableDataSource<PurchaseOrder>(this.purchase_orders);
